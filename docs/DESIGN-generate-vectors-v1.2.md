...
**不继承数据，但继承技术陷阱的经验。** v1.0/v1.1 的独立审计报告（2026-07-27）暴露了 10 个技术陷阱 + 4 条额外约束。详见 `DESIGN-vector-inventory-v1.2.md` Part A。关键要求：

- T1: 生成脚本必须端到端可复现（单一入口，禁止手动编辑 JSON）
- T2: 禁止内部状态字段（`__placeholder` 等）泄漏到输出 — `stripInternalFields()` 最终过滤
- T3: description 与实际内容自动校验 — `validateVectorSet()` 自检
- T4: AV-008 的 vector_ref 指向真实存在的 ID（`AV-003`，非假 ID）
- T5: 彻底移除 `expected_sha256` 答案密钥
- T9: 严格遵守 `delete obj.field` 而非 `obj.field = null`（JCS: Omit over Null）

**继承测试场景的设计模式**（不继承具体数据）：v1.0/v1.1 的 security-baseline、compliance-workflow、ring-0-precedence、all-operators 等场景类别在 v1.2 中得到保留和扩展。

### 参考架构

架构模式参考 v1.1 `generate-vectors.cjs` 的干净设计：
- 单一 `.cjs` 文件，`node generate-vectors.cjs` 一键运行
- 依赖 `json-canonicalize` (npm) 进行 JCS 序列化
- 输出 `decision-object-vectors-v1.2.json`

**不参考**：v1.1 的向量数据内容（规则定义、context、expected 值）— 全部从零编写。

### 关联文档

- **向量完整清单**: `DESIGN-vector-inventory-v1.2.md` — 63 条 DO + 26 条动态 + 12 条 AV 的逐条定义
- **技术陷阱继承**: 同上，Part A
...
```
静态决策向量    63 条    13 种决策类型 x 5-6 种代表性场景
动态决策向量    26 条    Temporal(10) + Seeded(8) + Stateful(8)
审计哈希向量    12 条    AV-001~AV-008 + AV-009~AV-012
─────────────────────
总计           101 条
```

**DELEGATE 预留**: DO-064 和 AV-013 预留至 v1.3（参考实现引擎未完成），在输出 JSON 中以 `reserved_vectors` 字段声明，不生成具体内容。

---

## 2. 决策类型清单（权威源：SPEC v1.1 §3.4 + 白皮书 §13.2）

SPEC v1.1 定义了 13 种**外部可见**决策类型：

| # | 决策类型 | Ring | v1.1 规范状态 | v1.2 向量覆盖 |
|---|---------|:---:|-------------|:---:|
| 1 | ALLOW | 3 | ✓ 已实现 | ✓ |
| 2 | DENY | 0 | ✓ 已实现 | ✓ |
| 3 | CORRECT | 3 | ✓ 已实现 | ✓ |
| 4 | REQUEST_HUMAN | 2 | ✓ 已实现 | ✓ |
| 5 | ESCALATE | 2 | ✓ 已实现 | ✓ |
| 6 | NOTIFY | 3 | ✓ SPEC 定义 | ✓ |
| 7 | EMERGENCY_HALT | 0 | ✓ 已实现 | ✓ |
...
> PASS: 虽然 SPEC 未将其列为"外部决策类型"，但向量集需要验证"无规则命中时 DO 仍正确生成"的场景，PASS 作为事实上的第 13 种 DO 结果类型。
...
| 决策类型 | 向量数 | 主要 DO 编号 | 覆盖场景 |
|---------|:---:|------------|------|
| ALLOW | 12 | DO-011,012,020,024,026,030,041~043,046~048 | 放行 + override安全方向 + override不安全方向 + unless豁免 + in/not_in/contains/lte/starts_with/ends_with + metadata fallback |
| DENY | 10 | DO-001,003,005,008,010,025,044,049,061,063 | 安全拦截 + 危险命令 + 关键系统 + 环0优先 + 严重性升级 + unless未豁免 + ReDoS + exists真值 + 对象深层比较 + 整数安全范围 |
| PASS | 10 | DO-002,004,006,007,014,027~029,050,062 | 规则选择性匹配 + 安全命令通过 + 规则集版本 + 空规则集 + 上下文差异 + null安全(neq/eq) + 严格类型 + exists缺失 + 空扩展规范化 |
| REQUEST_HUMAN | 4 | DO-013,015,016,045 | PII审批 + 非工作时段 + HIPAA医疗 + 高风险评分 |
| NOTIFY | 4 | DO-031~034 | 异常通知 + 操作审计 + 阈值预警 + 伴随DENY |
| WORKFLOW | 4 | DO-051~054 | 多步骤 + 条件分支 + 审批节点 + 完成 |
| ESCALATE | 3 | DO-017~019 | 低信誉Agent + 跨域 + 未知工具 |
| CORRECT | 3 | DO-021~023 | 大小写纠偏 + 单位转换 + 路径规范化 |
...
| WORKFLOW_PROGRESS | 3 | DO-058~060 | 步骤推进 + 阶段完成 + 最终步骤 |
| EMERGENCY_HALT | 1 | DO-009 | 环0紧急终止（短路所有评估 + 全局生效） |
| **总计** | **63** | DO-001~DO-063 | |

> 注：运算符覆盖（13 种）系统性地分布在 DO-041~050 区间，边缘穷尽（7 种）分布在 DO-027~029、DO-044、DO-049、DO-061~063。两者都已经计入上述决策类型的分配中，不是独立的 8 条。

### 运算符覆盖映射（13 运算符 → DO 编号）
...
exists   → DO-049 (DENY: context.high_risk_flag exists and is true)
  +      → DO-050 (PASS: context.high_risk_flag 不存在 → 不触发)
```
...
向量集验证全功能兼容性，故使用全激活模式——所有 JURISDICTION 字段都填充常量值：
...
| `agent.aid` | `"91110108MA12345678A00000001E"` | 白皮书 附录 A |
| `agent.known_limitations` | `["This is a test runner; does not perform real operations","Timeout after 30s for contexts >10KB"]` | 白皮书 附录 A |
...
**注**: `human_oversight` 是 CORE #13，不在 `activated_fields` 中。所有 DO 必然携带，无需激活。
...
Step 0: 构建 compliance_profile（不含 profile_hash）
        → JCS(cp) → SHA-256 → 写回 profile_hash
...
从 63 条静态 DO 中选 12 条生成 AV，覆盖 11 种决策类型 + AV-008 回归：
...
| AV-008 | DO-011 | (回归) | 陈旧回归向量（canonical_bytes 同 AV-003, audit.hash 保留旧值） |
...
| 等级 | 向量数 | 含义 | 来源 |
|:---:|:---:|------|------|
| L1 Basic | 28 | v1.0 等价范围 (早期决策类型) | SPEC v1.1 §12 冻结的 v1.0 DO 定义 |
| L2 Verified | 45 | v1.1 等价范围 (37 DO + 8 AV) | v1.1 向量集规模 |
| L3 Full | 101 | v1.2 全部 (63 DO + 26 dynamic + 12 AV) | 白皮书 §13.2 |
...
生成器依赖 `json-canonicalize`。验证器（`verify.js`）零依赖。
