...
`decision_divergence` is a **cross-layer semantic re-derivation check** (uses the deterministic evaluator `@openoba/erdl`, pinned by the 240 V-ENGINE vectors; its object is the hash-layer DO's decision field), not a hash-layer/field check (P1–P6):
