...
> 版本: 1.3 · 2026-07-29
> 状�? Released
> 目标: 跨语言、跨实现、零依赖�?DO 验证器，支持 L1/L2/L3 三级兼容
...
| 目标 | 说明 |
|------|------|
| **零依�?* | 只用 Node.js 内置模块 (`crypto`, `fs`, `path`)，不依赖 npm |
| **单一文件** | `scripts/verify.js`，可以直接复制到任何 Node.js 环境运行 |
| **三级兼容** | L1 Basic (28) / L2 Verified (45) / L3 Full (101) |
| **可移�?* | 纯算法实现，可翻译为 Python/Go/Rust |
| **自文�?* | `--help` 输出完整的验证方法说�?|

## 2. 核心算法: 五步验证�?
### 2.1 自建 JCS (不依赖外部库)

`verify.js` 必须自建 JCS 序列化器，因�?

1. 验证器的权威性建立在"不信任任何外部库"的基础�?2. 验证器本身是算法�?活文�?——任何人阅读代码就能理解 DO 的验证逻辑
3. 与实际生产用�?`json-canonicalize` 互相制衡——如果两者结果不同，说明其中一个有 bug
...
function jcsCanonicalize(value) {
  ...
  if (typeof value === 'object') {
    const keys = Object.keys(value).sort(); // �?Unicode 码点排序
    return '{' + keys.map(k =>
...
**精度校验**: 验证器必须能检测到自建 JCS 与参�?`json-canonicalize` 的不同。通过 101 条向量的 `canonical_bytes` 预计算值进行比较�?
**Omit over Null 原则**：可选字段值为 `null`、`undefined` �?`[]` 时必须从 JSON 对象中物理删除（`delete obj.field`），而非保留 `"field": null`。JCS 序列化器在遍历对象键时自然会跳过已删除的键。此原则同样适用于验证器�?JSON 文件读取 DO 后的预处理——生成器保证不会写入 null 值字段，但验证器需要防御性地处理外部输入�?
### 2.2 数字规范�?(ES6 Number.prototype.toString)

**JCS (RFC 8785) = ECMAScript 数字序列化�?*

JCS §3.2.2.3 明确规定数字序列化与 ECMAScript (ECMA-262) �?`Number.prototype.toString()` 完全一致。这意味着�?- 整数：直接输�?`String(n)` �?`"3"`、`"0"`、`"-1"`
- 浮点数：输出 `sign + digits + . + digits + e + sign + digits` 格式

�?JavaScript 中，最简单的正确实现就是直接使用 `String(number)` �?`number.toString()`�?
```javascript
...
String(5e-324)      // �?"5e-324" (最小正�?
String(1.7976931348623157e+308) // �?"1.7976931348623157e+308" (最大正�?
```

**注意**：`String(0)` �?`String(-0)` 都输�?`"0"`（JCS 要求——零的正负号不被保留）。`NaN` �?`Infinity` �?JCS 禁止，必须在输入时被截获�?
```javascript
...
**验证器的职责**：验证器自建 JCS 的序列化结果必须�?`json-canonicalize` npm 包逐字节一致。如果某条向量的 `canonical_bytes` 匹配上了 `json-canonicalize` 的输出，就说明自�?JCS 是正确的�?
## 3. CLI 接口
...
function verifyAuditVector(av) {
  // 1. 深拷贝决策对�?  const clone = JSON.parse(JSON.stringify(av.decision_object));

  // 2. 提取 claimed hash
  const claimedHash = clone.audit.hash;

  // 3. 删除自引�?签名字段（仅�?audit.hash，保�?previous_hash �?commitment�?  delete clone.audit.hash;     // 只删 hash �?  delete clone.signature;
  delete clone.signing_key_id;

  // 4. JCS 序列化（CORE + JURISDICTION + EXTENSIONS�?  const canonicalStr = jcsCanonicalize(clone);
...
function filterByLevel(vectors, level) {
  ...
  // L1/L2 计数从向量集 metadata.compatibility_levels 动态读�?  // 生成器在输出 JSON �?metadata 中声明各级别计数
...
## 5. 可移植性设�?
`verify.js` 的代码结构遵�?**Literal Translation** 原则:

1. 不使用箭头函数以外的 ES6 特�?(可翻译为 Python/Go)
2. `jcsCanonicalize()` 是纯函数递归 —�?对应 Python �?`def jcs_canonicalize(value)`
3. `sha256()` �?`crypto.createHash('sha256')` —�?对应 Python �?`hashlib.sha256()`
4. 不依赖任�?Node.js 特有�?API 模式 (�?Stream/Buffer 高级功能)
...
| 场景 | 行为 |
|------|------|
| 向量文件不存�?| 退出码 1，输�?`ERROR: Vector file not found` |
| 向量格式无效 (缺少 audit_vectors) | 退出码 2，输�?`ERROR: Invalid vector format` |
| AV-013 �?expected MISMATCH | 状�?`EXPECTED_FAIL`，不计入总失败数 |
| extensions 验证 | 直接参与�?JCS，不需要单独验�?|
| canonical_bytes 不一�?| 状�?`FAIL`，输出差异长度和位置 |
| 自建 JCS �?reference 不同 | 状�?`PASS` 但输�?WARNING: `JCS divergence detected` |
| JCS 遇到 NaN/Infinity | 抛出: 退出码 3，输�?`INVALID: NaN/Infinity in JCS input` |

## 7. 函数签名 (完整 API)

```javascript
// 核心 JCS 序列�?function jcsCanonicalize(value) �?string
...
// CLI 主入�?function main(args) �?exitCode
```
...
| �?| 含义 |
|:---:|------|
| 0 | 全部 PASS (�?AV-013 expected failure) |
| 1 | 文件 I/O 错误 |
| 2 | 向量格式无效 |
| 3 | JCS 输入违规 (NaN/Infinity) |
| 4 | 有真正的 FAIL (�?expected failure) |

## 9. 文件大小预估

`verify.js` 单文件约 **400-500 �?*:
- JCS 序列化器: ~70 �?- SHA-256 封装: ~5 �?- 五步验证逻辑: ~60 �?- CLI 解析 + 格式化输�? ~100 �?- 注释 + 文档字符�? ~100 �?
## 10. 测试策略

### 10.1 自验�?(smoke test)

```bash
# 生成向量�?node scripts/generate-vectors.cjs
...
```bash
# Node.js 验证�?node scripts/verify.js > node-result.txt
...
# 比较: 三个结果�?canonical_bytes 必须逐字节一�?diff node-result.txt python-result.txt  # should be empty
diff python-result.txt go-result.txt    # should be empty
```

### 10.3 回归检�?
```bash
# v1.2 �?canonical_bytes �?v1.1 不同是预期的 (平面哈希变更)
# �?v1.1 的旧向量�?v1.2 下重算应有明确的差异记录
node scripts/verify.js --vectors=../erdl-vectors-v1.1/decision-object-vectors-v1.1.json 2>&1
# 期望: JCS 方法不同 �?大部�?audit.hash 不匹�?�?记录�?v1.1 legacy"
```
