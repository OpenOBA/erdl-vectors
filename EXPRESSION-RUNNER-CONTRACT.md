...
> This contract is the **authoritative definition** of "normative behavior" for an expression-layer runner. A conforming runner implements the ERDL expression kernel from first principles (spec + this contract) and recomputes the 236 `v-engine-vectors.json` vectors — independent of the reference implementation and of this repo's `scripts/verify-v-engine.mjs` (which is a second source of the reference implementation, not a third party).
...
For all **236** `v-engine-vectors.json` vectors, recompute and produce a `value` **value-identical** to `expected.value` (per `value_type`):
...
2. **Per-vector verification**: run all 236 `v-engine-vectors.json` vectors, `value` value-identical (ER4);
