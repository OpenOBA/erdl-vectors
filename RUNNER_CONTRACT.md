...
5. **提交注册（自动）**：PR 到 `submissions/`；CI 交叉验证通过后，合并时自动登记于 [IMPLEMENTATIONS.md](IMPLEMENTATIONS.md) 注册表（`scripts/update-registry.cjs` 从 `submissions/*.json` 派生，未通过的不登记）。
...
参考 runner 已将三条不变式实现为硬失败守卫，并在报告中输出覆盖面（`107/107 适用 DO + 1 N/A`，唯一 N/A = `V-DO-v15-C07[1]` 版本门）。
