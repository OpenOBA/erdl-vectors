> ⚠️ **归档声明（2026-08-18）**：本 RFC 已被 **OPENOBA-DOBJ-RFC-002**（Decision Object v1.4）取代，不再是现行标准。保留于 archive/ 作为历史记录：v1.3 全 DO 原像模式 + §3.1 通用序列化约束（由 RFC-002 §7 继承）+ §13.3 嵌套五步验证法（仅供历史档案 DO 验证）。v1.3 冻结向量（AV-001~013）保留为历史档案 + JCS 规范化层回归套件。
