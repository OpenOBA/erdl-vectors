...
- **Christopher Hopley (chopmob-cloud / AlgoVoi)** — independent technical critic. His in-depth review of the ERDL Decision Object v1.3 draft identified critical issues including self-referential hash exclusion gaps, cross-engine string-to-number canonicalization inconsistencies, and layered integrity weaknesses, directly leading to the adoption of the flat hashing architecture and security hardening. His deep understanding of JCS RFC 8785 canonicalization and compliance auditing shaped the protocol's rigor.
- **Erik Newton (Concordia)** — the first independent Runner implementer and the originator of the principle "neutrality is tested, not declared." He established the "three independent implementations, one open specification, no single owner" standardization path in A2A Discussion #2031, laying the methodological foundation for ERDL's evolution from open-source project to infrastructure standard. His Python-based Decision Object verification engine confirmed the first 5 audit vectors (AV-001~AV-005) byte-for-byte across two independent implementations, later extended to all 28 compliance vectors, proving that JCS+SHA-256 cross-implementation verification works in practice, not just in theory. During the v1.1 freeze-period audit, he independently identified the structural risk of `expected_sha256` as an answer key, proposing the "remove answer keys + retain canonical_hex as diagnostic tool + introduce stale regression vectors" approach, which has been fully adopted. His byte-by-byte comparison via `canonical_hex` pinpointed the AV-003/AV-004/AV-005 mismatch to a single em-dash character, proving `canonical_hex`'s irreplaceable value as a cross-implementation diagnostic anchor.
