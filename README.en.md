...
- **Erik Newton (Concordia)** — the first independent Runner implementer. His Python-based Decision Object verification engine confirmed the first 5 audit vectors (AV-001~AV-005) byte-for-byte across two independent implementations, proving that JCS+SHA-256 cross-implementation verification works in practice, not just in theory. He proposed the stale regression canary strategy (AV-008) — deliberately placing a vector whose audit.hash does not match its actual content, designed to catch lazy validators that skip the five-step verification and compare pre-computed hashes. He also identified the structural risk of pre-computed answer keys (expected_sha256) — if vectors embed their own answers, validators may read rather than independently compute hashes — driving v1.2's complete removal of answer keys from the vector file.
